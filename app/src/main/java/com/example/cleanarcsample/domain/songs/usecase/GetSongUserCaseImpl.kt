...
class GetSongUserCaseImpl @Inject constructor(
    ...
    override suspend fun getSong(keyword: String, offset: Int, limit: Int): Resource<List<SongEntity>> {
        return Resource.Success(mapper.map(songRepository.getSong(keyword, offset,limit).data?.results!!),UIStatus.SUCCESS)
    }
}